class Solution {
    ...
    /**
    Things Leraned
    1.for initiating as hashset with list of chars or strings we have to pass the list
    of it which is obj from wrpper class
    (List<String> list = Arrays.asList("a", "b", "a");
    Set<String> set = new HashSet<>(list);)
    2.setObj.contains(element)//return true or false if exits in hashset
    3.stringBuilderObj.reverse()//can reverse the string
    4.Character.isLetterOrDigit(ch)//checks if given char is letter or digit and if neither then return false
   5.Character.toLowerCase(c1)//to convert tthe char to lowerCase (c1.toLowerCase()//wrong)

    Efficient apprach (use 2 pointers starte and end)
